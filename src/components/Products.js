  ...
  const performAPICall = async () => {
    ...
    try{
      ...
      if(e.response && e.response.status === 500){
        ...
        // console.log("error"+ e);
        enqueueSnackbar("Could not fetch productsthat the backend is running, reachable and return valid JSON", {
          variant:'error'
        })
      }
    }
...
const fetchCart = async(token) => {
  ...
  try{
    ...
    if(error.response && error.response.status === 400){
      ...
      enqueueSnackbar("Could not fetch cart Details. Check that the backend is running, reacable and returns valid JSON.",{variant: "error"});
    }
    return null;
  }
}
  ...
  return (
    <div>
      ...
      />
       ...
       <Grid container>
         <Grid item xs={12} md={token && productsData.length ? 9 : 12} className="product-grid">
           <Box className="hero">
